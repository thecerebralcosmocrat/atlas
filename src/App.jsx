...
function OwnershipPanel({ repository }) {
  ...
  return (
    <div className="mt-6 rounded-[calc(var(--radius)+0.75rem)] border border-border bg-card p-3 shadow-sm">
      ...
      {!error && file && (
        <div className="mx-1 flex flex-col gap-2 rounded-lg bg-muted/40 p-3 text-xs">
          <div className="font-medium text-muted-foreground">
            {file.commitCount === 0
          ...
          <ul className="flex flex-col gap-1">
            {file.authors.map((author) => (
              <li key={author.email || author.name} className="flex items-center gap-2">
          ...
          <ul className="mt-1 flex flex-col gap-1">
            {hotspots.map((hotspot) => (
              <li key={hotspot.path} className="flex items-center gap-2">
                <span className="truncate text-foreground">{hotspot.path}</span>
                <span className="ml-auto shrink-0 text-[10px] uppercase tracking-wide text-muted-foreground">
                  {hotspot.commitCount}{" "}
